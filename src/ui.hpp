...
class ArrowButton {
  public:
    ...
    void detectClick(glm::vec2 pos) {
        ...
        glm::vec2 center = (pp1 + pp2 + pp3) / 3.f;

        glm::vec2 d1 = imguiCoords - pp1;
        glm::vec2 d2 = imguiCoords - pp2;
        glm::vec2 d3 = imguiCoords - pp3;
        glm::vec2 dc = imguiCoords - center;
        if (length(d1) < 75 || length(d2) < 75 || length(d3) < 75 || length(dc) < 75) {
...
/// Good code would use inheritance but im lazy af
class Button {
  public:
    Button(const std::shared_ptr<State> &state, ImVec2 pos, float sizeX, float sizeY)
        : state(state), size(std::min(sizeX, sizeY)) {
        p1 = {pos.x + sizeX / 2, pos.y + sizeY / 2};
        p2 = {pos.x - sizeX / 2, pos.y + sizeY / 2};
        p3 = {pos.x - sizeX / 2, pos.y - sizeY / 2};
        p4 = {pos.x + sizeX / 2, pos.y - sizeY / 2};
    }

    void draw(ImDrawList *drawList) {
        // Draw arrow
        drawList->AddQuadFilled(p1, p2, p3, p4, IM_COL32(255, 255, 255, 100 + 155 * (progress > 0 ? 1 : 0)));
        ...
        // Draw progress circle
        if (progress > 0.f) {
            ImVec2 center = ImVec2((p1.x + p2.x + p3.x + p4.x) / 4.f, (p1.y + p2.y + p3.y + p4.y) / 4.f);
            drawList->AddCircle(ImVec2(center.x, center.y), size / 2.5, IM_COL32(100, 100, 100, 255), 64, 5.0f);

            float start_angle = 0.0f;
            float end_angle = progress * 2.0f * M_PIf; // Convert progress to radians
            drawList->PathArcTo(center, size / 2.5, start_angle, end_angle, 64);
            if (progress >= 1.f) {
                drawList->PathStroke(IM_COL32(0, 0, 255, 150), false, 10.0f);
            } else {
                drawList->PathStroke(IM_COL32(0, 255, 0, 150), false, 10.0f);
    ...
    void detectClick(glm::vec2 pos) {
        ...
        glm::vec2 pp2 = {p2.x, p2.y};
        glm::vec2 pp3 = {p3.x, p3.y};
        glm::vec2 pp4 = {p4.x, p4.y};
        glm::vec2 center = (pp1 + pp2 + pp3 + pp4) / 4.f;

        glm::vec2 d1 = imguiCoords - pp1;
        glm::vec2 d2 = imguiCoords - pp2;
        glm::vec2 d3 = imguiCoords - pp3;
        glm::vec2 d4 = imguiCoords - pp4;
        glm::vec2 dc = imguiCoords - center;
        if (length(d1) < 75 || length(d2) < 75 || length(d3) < 75 || length(d4) < 75) {
            progress += 1.2f * ImGui::GetIO().DeltaTime;
            lastInteractionTime = ImGui::GetTime();
        }
