...
ModelRenderer::ModelRenderer(const std::shared_ptr<State> &state, const std::string &filename) : state(state) {
...
void ModelRenderer::render_ztesting(glm::mat4 proj, glm::mat4 view, glm::mat4 model_mat, float opacity,
                                    GLuint middleDepthTex, glm::mat4 middleView, glm::mat4 middleProj,
    ...
    glUniform2f(glGetUniformLocation(shader_.pid, "depth_resolution"), static_cast<float>(depthWidth),
...
void ModelRenderer::bindMesh(std::map<int, GLuint> &vbos, tinygltf::Model &model, tinygltf::Mesh &mesh) {
    ...
    for (size_t i = 0; i < mesh.primitives.size(); ++i) {
        ...
        if (model.textures.size() > 0) {
            ...
            if (tex.source > -1) {
                ...
                GLuint texid;
                glGenTextures(1, &texid); // TODO: check this
...
// bind models
void ModelRenderer::bindModelNodes(std::map<int, GLuint> &vbos, tinygltf::Model &model, tinygltf::Node &node) {
        ...
        for (const auto &primitive : mesh.primitives) {
            for (const auto &attrib : primitive.attributes) {
                ...
                glVertexAttribPointer(loc, accessor.type, accessor.componentType,
                                      accessor.normalized ? GL_TRUE : GL_FALSE, stride,
            ...
            if (!model.textures.empty()) {
                ...
                if (tex.source > -1) {
                    ...
                    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, image.width, image.height, 0, format, type,
                                 image.image.data()); // ✅ <-- This is critical!
                }
            }
        }
    }
...
void ModelRenderer::drawMesh(const std::map<int, GLuint> &vbos, tinygltf::Model &model, tinygltf::Mesh &mesh) {
    for (size_t i = 0; i < mesh.primitives.size(); ++i) {
        tinygltf::Primitive primitive = mesh.primitives[i];
        tinygltf::Accessor indexAccessor = model.accessors[primitive.indices];
...
void ModelRenderer::dbgModel(tinygltf::Model &model) {
    for (auto &mesh : model.meshes) {
        ...
        for (auto &primitive : mesh.primitives) {
            ...
            std::cout << "indexaccessor: count " << indexAccessor.count << ", type " << indexAccessor.componentType
                      << std::endl;
