...
uniform sampler2D depth_tex_middle;
...
uniform mat4 MVP;
uniform mat4 middle_proj;
uniform mat4 middle_view;
...
void main() {
    ...
    vec4 fragWorld = inverse(MVP) * vec4(position, 1.0);
    fragWorld /= fragWorld.w;

    // Transform to middle camera clip space
    vec4 midClip = middle_proj * middle_view * fragWorld;
    midClip /= midClip.w;

    // Convert to texture UV space
    vec2 uv = midClip.xy * 0.5 + 0.5;

    if (uv.x < 0.0 || uv.x > 1.0 || uv.y < 0.0 || uv.y > 1.0)
        discard;

    float midDepth = texture(depth_tex_middle, uv).r;
    float fragDepth = midClip.z * 0.5 + 0.5;

    if (fragDepth > midDepth + 0.005)
        discard;
